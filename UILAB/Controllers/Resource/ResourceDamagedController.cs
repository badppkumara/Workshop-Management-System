...
namespace UILAB.Controllers.Resource
{
    ...
    public class ResourceDamagedController : Controller
    {
        ResourceUserTR objresourceuser = new ResourceUserTR();
        IResource _IResource;

        public ResourceDamagedController()
        {
        ...
        public ActionResult List()
        {
            ...
            ViewBag.CurrentSub = "Damaged";
            TempData["Error"] = "";
            int _Segment = Convert.ToInt32(HttpContext.Session["Segment"]);

            using (var db = new DatabaseContext())
            {
                var list = (from data in db.ResourceDamage where data.SegmentID == _Segment orderby data.DamagedID ascending select data).ToList();
                return View(list);
            }
        }
        ...
        public ActionResult Create(ResourceUserTR item)
        {
            ...
            using (var db = new DatabaseContext())
            {
                ...
                else
                {
                    ...
                    _IResource.InsertResourceUser(objresourceuser);
