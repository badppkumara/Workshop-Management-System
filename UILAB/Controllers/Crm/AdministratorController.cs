...
namespace UILAB.Controllers.Crm
{
    ...
    public class AdministratorController : Controller
    {
        ...
        {
            if (Convert.ToInt32(HttpContext.Session["AdminUser"]) > 0)
            {
            ...
            ViewBag.CurrentSub = "Administrator";
            TempData["Error"] = "";
            int _Segment = Convert.ToInt32(HttpContext.Session["Segment"]);

            using (var db = new DatabaseContext())
            {
                var list = (from data in db.vw_UserSecurities where data.SegmentID == _Segment orderby data.UserName ascending select data).ToList();
                return View(list);
            }
        }
